...
class SettingsStore(context: Context) {
    ...
    fun read(): XrSettings = runCatching {
        ...
        XrSettings(
            ...
            sbs = j.optBoolean("sbs", false), ipdMm = j.optDouble("ipd", 63.0).toFloat(), fovDegrees = j.optDouble("fov", 75.0).toFloat(),
            renderScale = j.optDouble("renderScale", .85).toFloat(), renderWidth = j.optInt("renderWidth", 1920), targetFps = j.optInt("fps", 60),
            frontCamera = j.optBoolean("frontCamera", false), spatialTracking = j.optBoolean("spatialTracking", false), passthrough = j.optBoolean("passthrough", true),
